...
export function DefaultRoutePanel({ showData, isPushing, handlePush }: ConfigPanelProps) {
            ...
            {defaultRoutes.map((route, idx) => (
                <div key={idx} className="bg-gray-50 rounded-lg p-4 space-y-3">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            {defaultRoutes.length > 1 && (
                                <button
                                    onClick={() =>
                                        setDefaultRoutes((prev) => prev.filter((_, i) => i !== idx))
                                    }
                    ...
                    <div className="flex justify-end gap-2">
                        <button
                            onClick={() =>
                                handlePush("routing.default.delete", { next_hop: route.next_hop })
                            }
                            disabled={isPushing || !route.next_hop}
                        ...
                        <button
                            onClick={() =>
                                handlePush("routing.default.add", { next_hop: route.next_hop })
                            }
                            disabled={isPushing || !route.next_hop}
